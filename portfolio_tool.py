...
# ============================================================
# SECTION: HELPER FUNCTIONS & BULLETPROOF GEMINI EXTRACTION
# ============================================================
def safe_float(v):
    try:
...
def process_pdf_ffs_with_gemini(uploaded_pdfs):
    ...
    # Candidate model endpoints ranked by preference for maximum compatibility
    candidate_models = [
    ...
    for idx, pdf_file in enumerate(uploaded_pdfs):
        status_text.text(f"⏳ Processing FFS ({idx + 1}/{total_files}): {pdf_file.name}...")
        
        pdf_bytes = pdf_file.read()
        
        prompt = """
        Analyze this Fund Fact Sheet (FFS) document and extract the following financial parameters into a strict, valid JSON object.
        Do not wrap the JSON in Markdown code blocks or formatting.
        ...
        If any metric or return figure is not present in the document, return null for that value.
        """

        parsed_successfully = False
        
        # Try candidate models sequentially until execution succeeds
        ...
        if not parsed_successfully:
            st.warning(f"⚠️ Could not fully parse {pdf_file.name}. Created default entry.")
            fallback_entry = {
                "Fund Name": pdf_file.name.replace(".pdf", ""),
                "1Y Return (%)": 5.0,
                "Volatility (%)": 10.0,
                "Mgmt Fee (%)": 1.5
            }
        ...
        # Pacing rate limit protection for Free Tier (10 RPM cap)
...
def optimize_portfolio_max_return(df, risk_profile):
    ...
    returns_col = df['1Y Return (%)'].fillna(df['3Y Return (%)']) if '3Y Return (%)' in df.columns else df['1Y Return (%)']
...
def calculate_portfolio_metrics(df, weights, year_returns_cols):
    returns_1y = df['1Y Return (%)'].fillna(df['3Y Return (%)']) if '3Y Return (%)' in df.columns else df['1Y Return (%)']
    ...
    if st.session_state.page == 'home':
        ...
        if st.session_state.portfolio_data:
            ...
            if st.button(" Return to Previous Analysis", use_container_width=True, type="primary", key="btn_return_analysis"):
                ...
                st.rerun()
            st.markdown("---")
            if st.button("🗑️ Clear Data & Start New Analysis", use_container_width=True, key="btn_clear_data"):
                st.session_state.portfolio_data = {}
                st.session_state.funds_df = None
                st.rerun()
            st.markdown("---")
        ...
        st.subheader("3. Select Data Input Method")
        input_method = st.radio("How would you like to provide fund data?", ["Upload Excel File", "Upload Fund Fact Sheets (PDF)"], horizontal=True)
        ...
        if input_method == "Upload Excel File":
            ...
            if st.button(" Download Excel Template", key="btn_dl_template"):
                ...
                st.download_button(label="Download Template Excel", data=buffer.getvalue(), file_name="fund_data_template.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        else:  # Upload Fund Fact Sheets (PDF)
            st.info("🤖 **AI Parsing:** Upload one or more PDF Fund Fact Sheets. Gemini API will automatically extract returns, fees, and volatility metrics.")
            uploaded_pdfs = st.file_uploader("Upload Fund Fact Sheets (PDF)", type=['pdf'], accept_multiple_files=True, key="pdf_uploader")
            
            if uploaded_pdfs and st.button("🤖 Process PDFs with Gemini AI", type="secondary", key="btn_process_pdfs"):
                extracted_df = process_pdf_ffs_with_gemini(uploaded_pdfs)
                if extracted_df is not None and not extracted_df.empty:
                    if '1Y Return (%)' not in extracted_df.columns: extracted_df['1Y Return (%)'] = 0.0
                    if 'Volatility (%)' not in extracted_df.columns: extracted_df['Volatility (%)'] = 10.0
                    if 'Mgmt Fee (%)' not in extracted_df.columns: extracted_df['Mgmt Fee (%)'] = 1.5
                    
                    extracted_df['1Y Return (%)'] = pd.to_numeric(extracted_df['1Y Return (%)'], errors='coerce').fillna(0.0)
                    extracted_df['Volatility (%)'] = pd.to_numeric(extracted_df['Volatility (%)'], errors='coerce').fillna(10.0)
                    extracted_df['Mgmt Fee (%)'] = pd.to_numeric(extracted_df['Mgmt Fee (%)'], errors='coerce').fillna(1.5)
                    
                    st.session_state.funds_df = extracted_df

        # Display Interactive Verification Table if Data is Loaded
        if st.session_state.funds_df is not None and not st.session_state.funds_df.empty:
            df = st.session_state.funds_df
            year_returns, year_benchmarks = detect_year_columns(df)
            
            st.markdown("---")
            st.subheader("🔍 Review & Verify Extracted Fund Data")
            st.caption("You can edit any cells directly in the table below before calculating your portfolio analysis.")
            
            edited_df = st.data_editor(df, use_container_width=True, num_rows="dynamic", key="data_editor_funds")
            st.session_state.funds_df = edited_df

            st.markdown("---")
            if st.button("Calculate Portfolio Analysis", use_container_width=True, type="primary", key="btn_calc_analysis"):
                st.session_state.portfolio_data = {
            ...
            st.warning("Please upload fund data using Excel or PDF Fact Sheets above to proceed.")
        ...
        if has_data or has_benchmark:
            ax3.set_xlabel("Year", fontsize=12, fontweight='bold'); ax3.set_ylabel("Return (%)", fontsize=12, fontweight='bold')
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            if any(pd.notna(r) for r in fund_returns):
                ...
                fund_has_data = True
        if fund_has_data:
            ax4.set_xlabel("Year", fontsize=12, fontweight='bold'); ax4.set_ylabel("Return (%)", fontsize=12, fontweight='bold')
        ...
        hdr_cells[0].text = 'Fund Name'; hdr_cells[1].text = '1Y Return (%)'; hdr_cells[2].text = 'Volatility (%)'
