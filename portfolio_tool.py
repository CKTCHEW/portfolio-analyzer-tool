  ...
  if key not in st.session_state:
    st.session_state[key] = (
        ...
        if key == "authenticated"
            ...
            None
            if key in ["user_email", "user_name", "otp_code", "otp_email"]
            else (
                "home"
                if key == "page"
                else None if key == "funds_df" else {}
            )
        )
    )
...
def process_pdf_ffs_with_gemini(uploaded_pdfs):
  ...
  if not api_key:
    st.error(
        "❌ GEMINI_API_KEY is missing in Streamlit Cloud Secrets. Please add"
        " it to proceed."
    )
    return None
      ...
      for model_name in candidate_models:
        try:
          ...
          if "Fund Name" not in data or not data["Fund Name"]:
            data["Fund Name"] = pdf_file.name.replace(".pdf", "").replace(
                "_", " "
            )
...
def optimize_portfolio_max_return(df, risk_profile):
  ...
  returns_col = (
      df["1Y Return (%)"].fillna(df["3Y Return (%)"])
