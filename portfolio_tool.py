...
# ============================================================
# SECTION: GEMINI PDF FFS EXTRACTION (UPDATED FOR google-genai)
# ============================================================
...
# ============================================================
# SECTION: MAIN APPLICATION FUNCTION
# ============================================================
def main():
  if not st.session_state.authenticated:
    ...
    if st.session_state.page == "home":
      ...
      with tab_excel:
        st.info(
            "Upload an Excel file with your fund data using the standard"
            " template."
        )
      ...
      eq_metrics = calculate_portfolio_metrics(
          df, equal_weights, year_returns_cols
      )
      ...
      opt_metrics = calculate_portfolio_metrics(
          df, opt_weights, year_returns_cols
      )
      ...
      if diff_monthly < -1:
        msg1 = (
            ...
            f" **${abs(diff_monthly):,.0f}** (New total:"
            f" ${req_monthly:,.0f}/month) and still meet your target."
        )
      elif diff_monthly > 1:
        msg1 = (
      ...
      if diff_initial < -1:
        msg2 = (
            "If monthly contribution is $0, you can **reduce** initial"
            f" investment by **${abs(diff_initial):,.0f}** (New total:"
        ...
        msg2 = (
            "If monthly contribution is $0, your initial investment is on track."
        )
        ...
        if year_return_col in df.columns:
          ...
          if valid_mask.any():
            eq_port_returns.append(
                np.sum(
                    df[year_return_col][valid_mask] * equal_weights[valid_mask]
                )
            )
      ...
      if st.button("⬅️ Back to Input Form", key="btn_back_input"):
        st.session_state.page = "input"
        st.rerun()
