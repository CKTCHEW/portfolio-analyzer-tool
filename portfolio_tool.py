...
def optimize_portfolio_with_all_funds(df, target_return, risk_profile):
    """
    Optimizes portfolio to maximize return while:
    1. Including ALL funds (never remove any)
    2. Staying within risk profile volatility threshold
    3. Each fund gets minimum 5% allocation
    """
    n = len(df)
    ...
    # Start with equal weights (ensures all funds are included)
    weights = np.ones(n) / n
    
    # Calculate current portfolio metrics
    current_return = np.sum(returns * weights)
    current_vol = np.sum(volatilities * weights)
    
    # If already within risk profile, try to optimize by tilting toward higher returns
    if current_vol <= max_volatility:
        # Sort by return (highest first)
        sorted_indices = np.argsort(returns)[::-1]
        
        # Try to increase weights on higher-return funds while staying within risk
        for idx in sorted_indices:
            # Try increasing weight by 5% increments
            for _ in range(8):  # Max increase of 40% (8 * 5%)
                test_weights = weights.copy()
                # Increase this fund's weight
                test_weights[idx] += 0.05
                # Decrease other funds proportionally
                other_indices = [i for i in range(n) if i != idx]
                total_other = np.sum(test_weights[other_indices])
                if total_other > 0:
                    scale_factor = (1.0 - test_weights[idx]) / total_other
                    test_weights[other_indices] *= scale_factor
                
                # Ensure minimum 5% for all funds
                test_weights = np.clip(test_weights, 0.05, 1.0)
                test_weights = test_weights / test_weights.sum()
                
                # Check if within risk profile
                test_vol = np.sum(volatilities * test_weights)
                if test_vol <= max_volatility:
                    weights = test_weights
                else:
                    break  # Can't increase further without exceeding risk
...
if not st.session_state.authenticated:
    ...
    if st.session_state.page == 'home':
        ...
        if st.session_state.portfolio_data:
            ...
            if st.button("🔄 Return to Previous Analysis", use_container_width=True, type="primary"):
                st.session_state.page = 'analysis'
                st.rerun()
            st.markdown("---")
            if st.button("🗑️ Clear Data & Start New Analysis", use_container_width=True):
                st.session_state.portfolio_data = {}
                st.session_state.funds_df = None
                st.rerun()
            st.markdown("---")
        ...
        # Optimize with all funds included
        opt_weights, opt_return, opt_vol = optimize_portfolio_with_all_funds(df, target_return, data['risk_profile'])
        ...
        # Display max volatility threshold
        risk_thresholds = {"Conservative": 10.0, "Moderate": 15.0, "Growth": 20.0}
        ...
        st.info(f"**Max Volatility for {data['risk_profile']} Profile:** {max_vol_threshold}%")
        ...
        # Calculate all options based on optimized portfolio
        ...
        if opt_amount > target_amount * 1.1:
            ...
            monthly_diff = data['monthly_contribution'] - reduced_monthly
            if monthly_diff > 0:
                st.info(f"**Option 1 - Adjust Monthly Contribution:**\nYou could reduce your monthly contribution from ${data['monthly_contribution']:,.0f} to ${reduced_monthly:,.0f} (save ${monthly_diff:,.0f}/month)\n\n*Note: This is optional as you are already exceeding your target.*")
            else:
                st.info(f"**Option 1 - Adjust Monthly Contribution:**\nCurrent: ${data['monthly_contribution']:,.0f}/month\n\n*Note: No reduction needed. You can maintain current contribution.*")
            
            if reduced_initial < data['initial_investment']:
                st.info(f"**Option 2 - Adjust Initial Investment:**\nIf you contribute $0/month, you could reduce initial investment from ${data['initial_investment']:,.0f} to ${reduced_initial:,.0f}\n\n*Note: This is optional as you are already exceeding your target.*")
            else:
                increase_amt = reduced_initial - data['initial_investment']
                st.info(f"**Option 2 - Adjust Initial Investment:**\nIf you contribute $0/month, you would need to increase initial investment from ${data['initial_investment']:,.0f} to ${reduced_initial:,.0f} (add ${increase_amt:,.0f})\n\n*Note: This shows the trade-off between monthly contributions and initial capital.*")
            
            years_saved = data['years'] - shortened_years
            if years_saved > 0:
                st.info(f"**Option 3 - Adjust Time Horizon:**\nYou could achieve your target in {shortened_years} years instead of {data['years']} years (save {years_saved} years)\n\n*Note: This is optional as you are already exceeding your target.*")
            else:
                st.info(f"**Option 3 - Adjust Time Horizon:**\nCurrent: {data['years']} years\n\n*Note: No reduction needed. You can maintain current timeline.*")
            ...
            years_added = increased_years - data['years']
            st.info(f"**Option 1 - Extend Time Horizon:**\nYou would need {increased_years} years instead of {data['years']} years (add {years_added} years)\n\n*Note: Action required to meet your target.*")
            
            monthly_increase = required_monthly - data['monthly_contribution']
            st.info(f"**Option 2 - Increase Monthly Contribution:**\nIncrease monthly contribution from ${data['monthly_contribution']:,.0f} to ${required_monthly:,.0f}/month (add ${monthly_increase:,.0f}/month)\n\n*Note: Action required to meet your target.*")
            
            initial_increase = required_initial - data['initial_investment']
            st.info(f"**Option 3 - Increase Initial Investment:**\nIncrease initial investment from ${data['initial_investment']:,.0f} to ${required_initial:,.0f} (add ${initial_increase:,.0f})\n\n*Note: Action required to meet your target.*")
        else:
            st.success(f"✅ **Target Met:** Your optimized portfolio achieves ${opt_amount:,.0f}, meeting your target of ${target_amount:,.0f}.")
            
            st.info(f"**Option 1 - Monthly Contribution:**\nCurrent: ${data['monthly_contribution']:,.0f}/month\n\n*Note: No action needed. You can maintain current contribution.*")
            
            st.info(f"**Option 2 - Initial Investment:**\nCurrent: ${data['initial_investment']:,.0f}\n\n*Note: No action needed. You can maintain current investment.*")
            
            st.info(f"**Option 3 - Time Horizon:**\nCurrent: {data['years']} years\n\n*Note: No action needed. You can maintain current timeline.*")
        ...
        with col1:
            ...
            wedges1, texts1, autotexts1 = ax1.pie(equal_weights, labels=df['Fund Name'], autopct='%1.1f%%', 
        ...
        with col2:
            ...
            wedges2, texts2, autotexts2 = ax2.pie(opt_weights, labels=df['Fund Name'], autopct='%1.1f%%', 
                                    startangle=90, colors=plt.cm.Paired.colors,
                                    pctdistance=0.85, labeldistance=1.1)
            ...
            for text in texts2:
                text.set_fontsize(8)
            ax2.set_title("Optimized Allocation (Risk-Constrained)", fontsize=12, fontweight='bold', pad=20)
            st.pyplot(fig2)
        ...
        doc.add_paragraph(f"Max Volatility Threshold: {max_vol_threshold}%")
        ...
        if opt_amount > target_amount * 1.1:
            ...
            monthly_diff = data['monthly_contribution'] - reduced_monthly
            if monthly_diff > 0:
                doc.add_paragraph(f"Option 1: Reduce monthly contribution from ${data['monthly_contribution']:,.0f} to ${reduced_monthly:,.0f} (save ${monthly_diff:,.0f}/month). (Optional)")
            else:
                doc.add_paragraph(f"Option 1: Maintain monthly contribution of ${data['monthly_contribution']:,.0f}. (No reduction needed)")
            
            if reduced_initial < data['initial_investment']:
                doc.add_paragraph(f"Option 2: If monthly contribution is $0, reduce initial investment from ${data['initial_investment']:,.0f} to ${reduced_initial:,.0f}. (Optional)")
            else:
                increase_amt = reduced_initial - data['initial_investment']
                doc.add_paragraph(f"Option 2: If monthly contribution is $0, increase initial investment from ${data['initial_investment']:,.0f} to ${reduced_initial:,.0f} (add ${increase_amt:,.0f}). (Shows trade-off)")
            
            years_saved = data['years'] - shortened_years
            if years_saved > 0:
                doc.add_paragraph(f"Option 3: Achieve target in {shortened_years} years instead of {data['years']} years (save {years_saved} years). (Optional)")
            else:
                doc.add_paragraph(f"Option 3: Maintain time horizon of {data['years']} years. (No reduction needed)")
                
        elif opt_amount < target_amount:
            doc.add_paragraph("⚠️ SHORTFALL SCENARIO: Your portfolio cannot meet the target.")
            years_added = increased_years - data['years']
            doc.add_paragraph(f"Option 1: Extend time horizon to {increased_years} years (add {years_added} years). (Action required)")
            
            monthly_increase = required_monthly - data['monthly_contribution']
            doc.add_paragraph(f"Option 2: Increase monthly contribution to ${required_monthly:,.0f} (add ${monthly_increase:,.0f}/month). (Action required)")
            
            initial_increase = required_initial - data['initial_investment']
            doc.add_paragraph(f"Option 3: Increase initial investment to ${required_initial:,.0f} (add ${initial_increase:,.0f}). (Action required)")
        else:
            doc.add_paragraph("✅ TARGET MET: Your optimized portfolio meets your investment goal.")
            doc.add_paragraph(f"Option 1: Maintain monthly contribution of ${data['monthly_contribution']:,.0f}. (No action needed)")
            doc.add_paragraph(f"Option 2: Maintain initial investment of ${data['initial_investment']:,.0f}. (No action needed)")
            doc.add_paragraph(f"Option 3: Maintain time horizon of {data['years']} years. (No action needed)")
        doc.add_paragraph()
