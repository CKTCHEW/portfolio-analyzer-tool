...
def extract_fund_data(text):
    """Extracts data from Malaysian FFS - tailored to actual document structure."""
    data = {
    ...
    # 1. Fund Name - Look for specific fund names
    if 'Income and Growth Fund' in text:
        data['fund_name'] = 'AM Income and Growth Fund'
    elif 'Manulife Investment U.S. Equity Fund' in text or 'Manulife US Equity' in text:
        data['fund_name'] = 'Manulife Investment U.S. Equity Fund'
    elif 'Eastspring Investments Small-Cap Fund' in text or 'EastSpring Small Cap' in text:
        data['fund_name'] = 'Eastspring Investments Small-Cap Fund'
    elif 'Affin Hwang Select Asia Pacific' in text:
        data['fund_name'] = 'Affin Hwang Select Asia Pacific (ex Japan) Dividend Fund'
    elif 'Principal Global Titans Fund' in text:
        data['fund_name'] = 'Principal Global Titans Fund'
    
    # 2. Returns - Look for performance tables
    # 1 Year Return - look for "1 year" or "1-Year" followed by number
    patterns_1y = [
        r'1[\s-]*[Yy]ear[^\d]*?([\d\.]+)',
        r'1[\s-]*[Yy][^\d]+?([\d\.]+)',
        r'1 Year[^\d]+?([\d\.]+)',
    ...
    # 3. Volatility - Look for "Volatility Factor" or "VF" or "3-year Fund Volatility"
    vol_patterns = [
        r'Volatility Factor[\s\(]*[^\d]*?([\d\.]+)',
        r'VF[^\d]+?([\d\.]+)',
        r'3-year Fund Volatility[^\d]+?([\d\.]+)',
        r'Volatility[^\d]+?([\d\.]+)',
    ]
    ...
    # 4. Management Fee
    fee_patterns = [
        r'Annual Management Fee[:\s]*(?:Up to |Max )?([\d\.]+)',
        r'Management Fee[:\s]*(?:Up to |Max )?([\d\.]+)',
        r'Management Fee[^\d]+?([\d\.]+)[\s]*%?\s*p\.a\.?',
    ]
    ...
    for year in [2016, 2017, 2018, 2019, 2020]:
        ...
        for pattern in patterns:
            ...
            if m:
                val = safe_float(m.group(1))
                if val is not np.nan and -100 < val < 200:
                    data[f'ret_{year}'] = val
                    break

    # 6. Sector & Geographic Allocation
    sectors = ['Technology', 'Financials', 'Healthcare', 'Consumer Discretionary', 'Consumer Staples', 
               'Industrials', 'Communication Services', 'Energy', 'Materials', 'Real Estate', 
               'Utilities', 'Manufacturing', 'Banking', 'Property', 'IT', 'Basic Materials']
    countries = ['United States', 'Malaysia', 'China', 'Japan', 'Europe', 'United Kingdom', 
                 'Taiwan', 'India', 'Australia', 'Hong Kong', 'Indonesia', 'Singapore']
    ...
    for c in countries:
        ...
        if m:
            val = safe_float(m.group(1))
            if 0 <= val <= 100:
                data['geographic_allocation'][c] = val
    ...
    if st.session_state.page == 'home':
        ...
        if uploaded_ffs:
            ...
            with st.spinner("Processing FFS documents..."):
                st.session_state.ffs_data = [extract_fund_data(f.read().decode('utf-8', errors='ignore')) for f in uploaded_ffs]
                # Create DataFrame for editing
                cols = ['fund_name', '1y_return', '3y_return', '5y_return', 'volatility', 'management_fee', 'ret_2016', 'ret_2017', 'ret_2018', 'ret_2019', 'ret_2020']
        ...
        if has_data:
            ...
            st.warning("No calendar year return data (2016-2020) was extracted from the FFS documents to plot the historical chart.")
