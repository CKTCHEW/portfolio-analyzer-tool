...
def extract_fund_data(pdf_file):
    """Extracts data from FFS using pdfplumber table extraction + text fallback."""
    data = {
    ...
    try:
        with pdfplumber.open(pdf_file) as pdf:
            ...
            # 1. Extract Fund Name from text
            lines = full_text.split('\n')
            for line in lines[:20]:
                line = line.strip()
                if any(kw in line.lower() for kw in ['fund', 'income', 'growth', 'equity', 'small', 'cap', 'dividend', 'titans', 'asia', 'pacific', 'manulife', 'eastspring', 'affin', 'principal']):
                    if 15 < len(line) < 80 and not line.startswith('Source') and not line.startswith('The'):
                        data['fund_name'] = re.sub(r'\s+', ' ', line).strip()
                        break
            
            # 2. Extract data from tables (more reliable than regex)
            for table in all_tables:
                ...
                for row in table:
                    ...
                    if '1 year' in row_text.lower() or '1 yr' in row_text.lower() or '1-year' in row_text.lower():
                        for cell in row:
                            if cell:
                                try:
                    ...
                    if '3 year' in row_text.lower() or '3 yr' in row_text.lower() or '3-year' in row_text.lower():
                        for cell in row:
                            if cell:
                                try:
                                    val = float(str(cell).replace('%', '').replace(',', '').strip())
                    ...
                    if '5 year' in row_text.lower() or '5 yr' in row_text.lower() or '5-year' in row_text.lower():
                        for cell in row:
                            if cell:
                                try:
                                    val = float(str(cell).replace('%', '').replace(',', '').strip())
                    ...
                    # Look for volatility
                    if 'volatility' in row_text.lower() or 'vf' in row_text.lower():
                        for cell in row:
                            if cell:
                                try:
                                    val = float(str(cell).replace('%', '').replace(',', '').strip())
                    ...
                    if 'management fee' in row_text.lower():
                        for cell in row:
                            if cell:
                                try:
                                    val = float(str(cell).replace('%', '').replace(',', '').strip())
                    ...
                    # Look for calendar year returns
                    for year in [2016, 2017, 2018, 2019, 2020]:
                        if str(year) in row_text:
                            for cell in row:
                                if cell and str(year) not in str(cell):
                                    try:
                                        val = float(str(cell).replace('%', '').replace(',', '').strip())
                                        if -100 < val < 200:
            ...
            # 3. Fallback: regex on text if tables didn't work
            if np.isnan(data['1y_return']):
                m = re.search(r'1[\s-]*[Yy]ear[^\d]*?([\d\.]+)', full_text)
                if m:
            ...
            if np.isnan(data['3y_return']):
                m = re.search(r'3[\s-]*[Yy]ear[^\d]*?([\d\.]+)', full_text)
                if m:
                    val = safe_float(m.group(1))
                    if 0 < val < 500:
                        data['3y_return'] = val
            ...
            if np.isnan(data['5y_return']):
                m = re.search(r'5[\s-]*[Yy]ear[^\d]*?([\d\.]+)', full_text)
                if m:
                    val = safe_float(m.group(1))
                    if 0 < val < 2000:
                        data['5y_return'] = val
            ...
            if np.isnan(data['volatility']):
                m = re.search(r'(?:Volatility Factor|VF|Volatility)[^\d]*?([\d\.]+)', full_text, re.IGNORECASE)
                if m:
                    val = safe_float(m.group(1))
                    if 0 < val < 100:
                        data['volatility'] = val
            
            if np.isnan(data['management_fee']):
                m = re.search(r'Management Fee[^\d]*?([\d\.]+)', full_text, re.IGNORECASE)
                if m:
                    val = safe_float(m.group(1))
                    if 0 < val < 10:
                        data['management_fee'] = val
