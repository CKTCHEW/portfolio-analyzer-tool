...
def extract_fund_data(pdf_file):
    """Extracts data from FFS using table-based extraction."""
    data = {
    ...
    try:
        ...
        with pdfplumber.open(pdf_file) as pdf:
            ...
            # 1. Extract Fund Name - usually at the top of page 1
            lines = full_text.split('\n')
            for line in lines[:20]:
                line = line.strip()
                # Look for fund name - usually contains "Fund" and is not a header/footer
                if 'fund' in line.lower() and len(line) > 10 and len(line) < 100:
                    # Skip lines with common descriptors
                    skip_words = ['annual', 'management', 'volatility', 'performance', 'distribution', 
                                 'factsheet', 'investment', 'objective', 'risk', 'portfolio', 'benchmark']
            ...
            # 2. Process tables for returns and other data
                    ...
                    for row in table:
                        ...
                        if '1 year' in row_str or '1yr' in row_str or '1y' in row_str:
                            for i, cell in enumerate(row):
                                if cell and isinstance(cell, str):
                                    ...
                                    if 0 < val < 200 and not np.isnan(val):
                                        data['1y_return'] = val
                                        break
                        ...
                        if '3 year' in row_str or '3yr' in row_str or '3y' in row_str:
                            for i, cell in enumerate(row):
                                if cell and isinstance(cell, str):
                                    val = safe_float(cell.replace('%', ''))
                                    if 0 < val < 500 and not np.isnan(val):
                                        data['3y_return'] = val
                                        break
                        ...
                        if '5 year' in row_str or '5yr' in row_str or '5y' in row_str:
                            for i, cell in enumerate(row):
                                if cell and isinstance(cell, str):
                                    val = safe_float(cell.replace('%', ''))
                                    if 0 < val < 2000 and not np.isnan(val):
                                        data['5y_return'] = val
                                        break
                
                # Look for Calendar Year Returns table
                    ...
                    for row in table:
                        ...
                        for year in [2021, 2020, 2019, 2018, 2017, 2016]:
                            if str(year) in row_str:
                                # Find the value next to the year
                                for i, cell in enumerate(row):
                                    if cell and isinstance(cell, str) and str(year) in cell:
                                        # Check next cell for value
                                        if i + 1 < len(row) and row[i+1]:
                                            val = safe_float(str(row[i+1]).replace('%', ''))
                                            if -100 < val < 200 and not np.isnan(val):
            ...
            # 3. Extract Volatility Factor from text
            vol_patterns = [
            ...
            for pattern in vol_patterns:
                ...
                if match:
                    ...
                    if 0 < val < 100 and not np.isnan(val):
                        data['volatility'] = val
                        break
            
            # 4. Extract Management Fee from text
            fee_patterns = [
            ...
            for pattern in fee_patterns:
                ...
                if match:
                    val = safe_float(match.group(1))
                    if 0 < val < 10 and not np.isnan(val):
                        data['management_fee'] = val
                        break
            
            # 5. Fallback - If returns not found, try direct regex on text
            if np.isnan(data['1y_return']):
                # Look for "1 Year" followed by a number
                match = re.search(r'1[\s-]*[Yy]ear.*?([\d\.]+)\s*%', full_text)
                if match:
                    val = safe_float(match.group(1))
                    if 0 < val < 200:
                        data['1y_return'] = val
            ...
            if np.isnan(data['3y_return']):
                match = re.search(r'3[\s-]*[Yy]ear.*?([\d\.]+)\s*%', full_text)
                if match:
                    val = safe_float(match.group(1))
                    if 0 < val < 500:
                        data['3y_return'] = val
            ...
            if np.isnan(data['5y_return']):
                match = re.search(r'5[\s-]*[Yy]ear.*?([\d\.]+)\s*%', full_text)
                if match:
                    val = safe_float(match.group(1))
                    if 0 < val < 2000:
                        data['5y_return'] = val
            
            # 6. Fallback for calendar returns
            if all(pd.isna(data[f'ret_{year}']) for year in [2021, 2020, 2019, 2018, 2017, 2016]):
                # Try to find "Year To Date" section
                ytd_match = re.search(r'Year To Date.*?\n(.*?)(?:\n\s*\n|$)', full_text, re.DOTALL | re.IGNORECASE)
                if ytd_match:
                    section = ytd_match.group(1)
                    # Look for numbers that could be returns
                    numbers = re.findall(r'([\d\.]+)\s*%', section)
                    if numbers:
                        # Assign numbers to years (most recent first)
                        years = [2021, 2020, 2019, 2018, 2017, 2016]
                        for i, num in enumerate(numbers[:6]):
                            if i < len(years):
                                val = safe_float(num)
                                if -100 < val < 200:
                                    data[f'ret_{years[i]}'] = val
