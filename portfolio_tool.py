...
def extract_fund_data(text):
    """Extracts data from FFS text based on common patterns in Malaysian fund factsheets."""
    data = {
    ...
    # 1. Fund Name - Look for common patterns
    # Try to find fund name from title or header
    name_patterns = [
        r'(?:Fund Name|Fund)[:\s]*([A-Z][a-z]+(?:\s+[A-Z]?[a-z]+)*(?:\s+Fund)?(?:\s+\([^)]+\))?)',
        r'^([A-Z][a-zA-Z\s]+(?:Fund|Trust))',
        r'(Income and Growth Fund[^\n]*)',
        r'(Manulife[^\n]*Equity Fund[^\n]*)',
        r'(Eastspring[^\n]*Small[- ]*Cap Fund[^\n]*)',
        r'(Affin Hwang[^\n]*Dividend Fund[^\n]*)',
        r'(Principal Global Titans Fund[^\n]*)',
    ]
    ...
    for pattern in name_patterns:
        ...
        if match:
            ...
            if 15 < len(name) < 100 and 'fund' in name.lower():  # Reasonable length
                data['fund_name'] = name
                break
    
    # 2. Returns (1Y, 3Y, 5Y) - Look for performance tables
    # Try different patterns for annualized returns
    returns_1y_patterns = [
        r'1[\s-]*[Yy]ear[:\s]*([\d\.]+)',
        r'1[\s-]*[Yy][:\s]*([\d\.]+)',
        r'1 Year[^\d]*?([\d\.]+)',
    ...
    # 3. Volatility - Look for VF or Volatility Factor
    vol_patterns = [
    ...
    for p in fee_patterns:
        ...
        if m: 
            val = safe_float(m.group(1))
            if 0 < val < 10:  # Sanity check (fees are usually 0.5-3%)
    ...
    for year in [2016, 2017, 2018, 2019, 2020]:
        ...
        for pattern in patterns:
            ...
            if m:
                val = safe_float(m.group(1))
                if val is not np.nan and -100 < val < 200:  # Sanity check
                    data[f'ret_{year}'] = val
                    break

    # 6. Sector & Geographic Allocation (Simplified extraction)
    sectors = ['Technology', 'Financials', 'Healthcare', 'Consumer Discretionary', 'Consumer Staples', 'Industrials', 'Communication Services', 'Energy', 'Materials', 'Real Estate', 'Utilities', 'Manufacturing', 'Banking', 'Property']
    countries = ['United States', 'Malaysia', 'China', 'Japan', 'Europe', 'United Kingdom', 'Taiwan', 'India', 'Australia', 'Hong Kong', 'Indonesia']
