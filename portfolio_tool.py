...
def extract_fund_data(text):
    """Extracts data from Malaysian FFS with patterns specific to your documents."""
    data = {
    ...
    # 1. Fund Name - Look for specific patterns
    name_patterns = [
        r'(Income and Growth Fund[^\n]*)',
        r'(Manulife Investment U\.S\. Equity Fund[^\n]*)',
        r'(Eastspring Investments Small-Cap Fund[^\n]*)',
        r'(Affin Hwang Select Asia Pacific \(ex Japan\) Dividend Fund[^\n]*)',
    ...
    # 2. Returns - Look for performance tables with specific patterns
    # 1 Year Return
    patterns_1y = [
        r'1[\s-]*[Yy]ear[:\s]+([\d\.]+)',
        r'1[\s-]*[Yy][^\d]+([\d\.]+)',
