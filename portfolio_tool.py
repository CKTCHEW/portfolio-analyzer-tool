...
# ============================================================
# Initialize Firebase (only once per session to prevent "already exists" error)
...
if not st.session_state.authenticated:
    ...
    if st.session_state.page == 'home':
        ...
        st.subheader("3. Fund Data Input")
        st.info("For accurate calculations, please use the CSV Template. You can also upload FFS (PDF) for reference.")
        
        # NEW: FFS (PDF) Upload
        uploaded_ffs = st.file_uploader("Upload Fund Factsheets (FFS - PDF)", type=['pdf'], accept_multiple_files=True)
        if uploaded_ffs:
            st.success(f"✅ {len(uploaded_ffs)} FFS PDF(s) uploaded for reference.")
            
        st.markdown("---")
        st.subheader("4. Fund Data (CSV)")
        uploaded_csv = st.file_uploader("Upload Fund Data (CSV)", type=['csv'])
        ...
        if uploaded_csv is not None:
            try:
                ...
                st.success("✅ CSV uploaded successfully! Please review/edit the data below before calculating.")
                ...
                if st.button(" Calculate Portfolio Analysis", use_container_width=True, type="primary"):
                    ...
                    st.session_state.portfolio_data['risk_profile'] = risk_profile
                    st.session_state.page = 'results'
                    st.rerun()
            except Exception as e:
                st.error(f"Error reading CSV file. Please ensure it matches the template. Error: {e}")
        else:
            st.warning("Please upload a CSV file to proceed with calculations.")
        ...
        with col1:
            if st.button("📄 Generate Word Report", use_container_width=True):
                ...
                doc.add_paragraph(f"Risk Profile: {data.get('risk_profile', 'N/A')}")
            ...
            if st.button(" Generate Excel Report", use_container_width=True):
                ...
                with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                    ...
                    summary = pd.DataFrame({
                        'Metric': ['Risk Profile', 'Initial Investment', 'Monthly Contribution', 'Years', 'Projected Value', 'Required CAGR', 'Projected CAGR'],
                        'Value': [data.get('risk_profile', 'N/A'), data['initial_investment'], data['monthly_contribution'], data['years'], int(projected_value), f"{required_cagr:.1f}%", f"{port_return:.1f}%"]
                    })
