...
def increment_access(email):
    ...
    try:
        ...
        if docs:
            ...
            # Update access count AND record the current login date/time
...
def calculate_required_cagr(target_sum, initial_investment, monthly_contribution, years):
    ...
    # Simple approximation for required return using numpy-like logic
    # FV = PV*(1+r)^n + PMT * (((1+r)^n - 1) / r)
    # We'll use a numerical solver (Newton-Raphson simplified)
...
def optimize_portfolio(funds_df):
    ...
    # V1: Equal weight is the baseline. 
    # We calculate the weighted average return and volatility.
    weights = np.ones(len(funds_df)) / len(funds_df)
    port_return = np.sum(funds_df['expected_return'] * weights)
    # Mock volatility calculation (simplified)
    ...
    if st.session_state.page == 'home':
        ...
        with col1:
            if st.button("📄 Generate Word Report", use_container_width=True):
                # (Simplified Word gen for V1 - mirrors your existing logic)
                doc = Document()
