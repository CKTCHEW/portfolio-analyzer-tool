...
def extract_fund_data(text):
    """Extracts data from Malaysian FFS with improved patterns."""
    data = {
    ...
    # 1. Fund Name - Look for specific patterns in Malaysian FFS
    lines = text.split('\n')
    
    # Try to find fund name from common locations
    name_candidates = []
    for i, line in enumerate(lines[:30]):  # Check first 30 lines
        line = line.strip()
        
        # Look for fund names with specific keywords
        if any(keyword in line.lower() for keyword in ['fund', 'trust', 'investment']):
    ...
    # Choose the best candidate (usually the longest meaningful one)
    if name_candidates:
        # Prefer names that contain actual fund identifiers
        for candidate in sorted(name_candidates, key=len, reverse=True):
            if any(word in candidate.lower() for word in ['income', 'growth', 'equity', 'fund', 'investment', 'select', 'global', 'asia', 'small', 'cap', 'dividend']):
                data['fund_name'] = candidate
                break
    
    # 2. Returns - Look for performance tables
    # Pattern for "1 Year" or "1Y" returns
    patterns_1y = [
        r'1[\s-]*[Yy]ear[:\s]+([\d\.]+)',
        r'1[\s-]*[Yy][^\d]+([\d\.]+)',
        r'1 Year[^\d]+([\d\.]+)',
    ...
    # Pattern for "3 Year" or "3Y" returns
    patterns_3y = [
        r'3[\s-]*[Yy]ear[:\s]+([\d\.]+)',
        r'3[\s-]*[Yy][^\d]+([\d\.]+)',
        r'3 Year[^\d]+([\d\.]+)',
    ...
    # Pattern for "5 Year" or "5Y" returns
    patterns_5y = [
        r'5[\s-]*[Yy]ear[:\s]+([\d\.]+)',
        r'5[\s-]*[Yy][^\d]+([\d\.]+)',
        r'5 Year[^\d]+([\d\.]+)',
    ...
    # 3. Volatility - Look for "Volatility Factor" or "VF"
    vol_patterns = [
    ...
    # Look for table patterns with years
    ...
    for c in countries:
        ...
        if c not in ['United States', 'United Kingdom']:  # Avoid partial matches
            if m:
                val = safe_float(m.group(1))
                if 0 <= val <= 100:
