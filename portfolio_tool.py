...
def extract_fund_data(pdf_file):
    ...
    try:
        ...
        with pdfplumber.open(pdf_file) as pdf:
            ...
            # 1. Extract Fund Name - look for it in first few lines
            lines = full_text.split('\n')
            for i, line in enumerate(lines[:15]):
                line = line.strip()
                # Look for fund name patterns - usually bold or at start of page
                if len(line) > 10 and len(line) < 100:
                    # Check if it looks like a fund name (contains fund-related keywords or is title case)
                    if any(kw in line.lower() for kw in ['fund', 'income', 'growth', 'equity', 'dividend', 'asia', 'pacific', 'global', 'select', 'opportunity']):
                        if not line.startswith('Source') and not line.startswith('The') and not line.startswith('An'):
                            # Clean up the fund name - remove common prefixes
                            fund_name = re.sub(r'\s+', ' ', line).strip()
                            # Remove common descriptors that aren't part of fund name
                            fund_name = re.sub(r'^An\s+', '', fund_name)
                            fund_name = re.sub(r'^A\s+', '', fund_name)
                            data['fund_name'] = fund_name
                            break
            
            # 2. Extract Annual Returns - look for "Annualised Return" or "Annual Return" table
            for table in all_tables:
                if not table:
                    continue
                    
                # Convert table to strings for searching
                table_str = ' '.join([str(cell) for cell in table if cell]).lower()
                
                # Look for Annualised Return table
                if 'annualised' in table_str or 'annual return' in table_str:
                    for row in table:
                        row_str = ' '.join([str(cell) for cell in row if cell]).lower()
                        # Extract 1Y, 3Y, 5Y returns
                        if '1 year' in row_str or '1yr' in row_str or '1y' in row_str:
                            for cell in row:
                                if cell and isinstance(cell, str):
                                    val = safe_float(cell.replace('%', ''))
                                    if 0 < val < 200:
                                        data['1y_return'] = val
                                        break
                        if '3 year' in row_str or '3yr' in row_str or '3y' in row_str:
                            for cell in row:
                                if cell and isinstance(cell, str):
                                    val = safe_float(cell.replace('%', ''))
                                    if 0 < val < 500:
                                        data['3y_return'] = val
                                        break
                        if '5 year' in row_str or '5yr' in row_str or '5y' in row_str:
                            for cell in row:
                                if cell and isinstance(cell, str):
                                    val = safe_float(cell.replace('%', ''))
                                    if 0 < val < 2000:
            ...
            # 3. Extract Calendar Year Returns
            for table in all_tables:
                if not table:
                    continue
                    
                table_str = ' '.join([str(cell) for cell in table if cell]).lower()
                if 'calendar year' in table_str or 'year to date' in table_str:
                    # Find the row with the returns data
                    for row in table:
                        row_str = ' '.join([str(cell) for cell in row if cell])
                        # Look for year numbers
                        for year in [2021, 2020, 2019, 2018, 2017, 2016]:
                            if str(year) in row_str:
                                # Extract the number next to the year
                                for i, cell in enumerate(row):
                                    if cell and isinstance(cell, str):
                                        if str(year) in cell:
                                            # Look at adjacent cells for the return value
                                            if i + 1 < len(row) and row[i+1]:
                                                val = safe_float(str(row[i+1]).replace('%', ''))
                                                if -100 < val < 200:
            ...
            volatility_pattern = r'Volatility Factor.*?([\d\.]+)'
            vol_match = re.search(volatility_pattern, full_text, re.IGNORECASE)
            if vol_match:
                val = safe_float(vol_match.group(1))
                if 0 < val < 100:
                    data['volatility'] = val
            
            # If not found, try alternative pattern
            if np.isnan(data['volatility']):
                vf_pattern = r'VF.*?([\d\.]+)'
                vf_match = re.search(vf_pattern, full_text, re.IGNORECASE)
                if vf_match:
                    val = safe_float(vf_match.group(1))
                    if 0 < val < 100:
                        data['volatility'] = val
            
            # 5. Extract Management Fee
            fee_pattern = r'Annual Management Fee.*?([\d\.]+)%'
            fee_match = re.search(fee_pattern, full_text, re.IGNORECASE)
            if fee_match:
                val = safe_float(fee_match.group(1))
                if 0 < val < 10:
                    data['management_fee'] = val
            
            # 6. Fallback - try to find returns from text if tables didn't work
            if np.isnan(data['1y_return']):
                pattern = r'1[\s-]*[Yy]ear.*?([\d\.]+)%'
