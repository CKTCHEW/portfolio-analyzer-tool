...
if not st.session_state.authenticated:
    ...
    if st.session_state.page == 'home':
        ...
        # If we already have analysis data, show option to return to it
        ...
        with col1:
            if st.button("← Back to Input", use_container_width=True):
                # Preserve analysis data, just change page
                st.session_state.page = 'input'
                st.rerun()
        with col2:
        ...
        if opt_amount > target_amount * 1.1:
            ...
            doc.add_paragraph(f"Option 1: Reduce monthly contribution from ${data['monthly_contribution']:,.0f} to ${reduced_monthly:,.0f}. (Optional)")
            ...
            doc.add_paragraph(f"Option 3: Achieve target in {shortened_years} years instead of {data['years']} years. (Optional)")
                
        elif opt_amount < target_amount:
            doc.add_paragraph("️ SHORTFALL SCENARIO: Your portfolio cannot meet the target.")
            doc.add_paragraph(f"Option 1: Extend time horizon to {increased_years} years. (Action required)")
            doc.add_paragraph(f"Option 2: Increase monthly contribution to ${required_monthly:,.0f}. (Action required)")
            doc.add_paragraph(f"Option 3: Increase initial investment to ${required_initial:,.0f}. (Action required)")
        else:
