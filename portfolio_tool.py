...
def detect_year_columns(df):
    ...
    for col in df.columns:
        # Match patterns like "2016 Return (%)", "2017 Return (%)", etc.
        if re.match(r'^\d{4}\s+Return\s+\(%\)$', col):
            year = int(col.split()[0])
            year_returns.append((year, col))
        # Match patterns like "2016 Benchmark (%)", "2017 Benchmark (%)", etc.
...
def optimize_portfolio_risk_constrained(df, target_return, risk_profile, target_exceeded=False):
    """
    Optimizes portfolio based on whether target is exceeded or not.
    If target_exceeded=True: Minimize volatility while maintaining at least target return
    If target_exceeded=False: Maximize return while staying within risk profile
    """
    n = len(df)
    ...
    if target_exceeded:
        ...
        for idx in sorted_indices:
            ...
            if test_vol <= max_volatility:
                ...
                if fund_vol > 0:
                    max_weight_for_risk = (max_volatility - np.nansum(np.array([volatilities[i] * opt_weights[i] if not np.isnan(volatilities[i]) else 0 for i in range(n)]))) / fund_vol
...
def calculate_portfolio_metrics(df, weights, year_returns_cols):
    ...
    # Calendar year metrics (from detected year columns)
    yearly_returns = []
...
if not st.session_state.authenticated:
    ...
    if st.session_state.page == 'home':
        ...
        if st.button("📥 Download Excel Template"):
            # Create template with column headings only (no data)
        ...
        # 2. Determine if target is exceeded and optimize accordingly
        eq_amount = calculate_future_value(data['initial_investment'], data['monthly_contribution'], data['years'], eq_metrics['return'])
        target_exceeded = eq_amount >= target_amount * 1.1  # 10% buffer
        
        # Risk-Constrained Optimized Portfolio
        ...
        # Get unique years from detected columns
        ...
        secondary_benchmark = benchmark_names[1] if len(benchmark_names) > 1 else None
        
        for year in years_list:
            # Find column names for this year
            ...
            if year_return_col in df.columns:
                ...
                if valid_mask.any():
                    eq_return = np.sum(df[year_return_col][valid_mask] * equal_weights[valid_mask])
            ...
            if year_return_col in df.columns:
                valid_mask = df[year_return_col].notna()
                if valid_mask.any():
                    opt_return = np.sum(df[year_return_col][valid_mask] * opt_weights[valid_mask])
            ...
            if year_bench_col in df.columns:
                ...
                if valid_mask.any():
                    bench_return = np.sum(df[year_bench_col][valid_mask] * equal_weights[valid_mask])
        ...
        if has_benchmark:
            benchmark_label = f"{primary_benchmark} (Composite)" if secondary_benchmark else primary_benchmark
        ...
        if has_data or has_benchmark:
            ...
            st.warning("No calendar year return data found in the uploaded Excel file.")
        ...
        for i, row in df.iterrows():
            ...
            if any(pd.notna(r) for r in fund_returns):
                ax4.plot(valid_years, [r for r in fund_returns if pd.notna(r)], marker='o', linewidth=2, 
