...
# ============================================================
# Firebase is required for login and usage tracking. If its service-account
# secret is absent or invalid, login will be blocked with a visible error.
...
if not st.session_state.get("firebase_initialized", False):
    try:
        ...
        st.session_state.firebase_init_error = None
...
FIRESTORE_COLLECTION = "user_usage"
try:
...
def _record_firebase_status(message):
    st.session_state["firebase_status_message"] = message


def _get_usage_document(email):
    if db is None:
        raise RuntimeError(
            "Firebase is not connected. Check FIREBASE_SERVICE_ACCOUNT in Streamlit Secrets."
        )
...
def get_user_stats(email):
    ...
    doc = _get_usage_document(email)
    if doc is None:
        # New users always start with the configured default limit.
        doc_ref, _write_result = db.collection(FIRESTORE_COLLECTION).add({
            "email": email,
            "access_count": 0,
            "generation_count": 0,
            "max_limit": DEFAULT_USER_LIMIT,
            "created_at": firestore.SERVER_TIMESTAMP,
        })
        _record_firebase_status(
            f"Firestore created user_usage/{doc_ref.id} in project "
            f"{firebase_project_id or 'UNKNOWN_PROJECT'} for {email}."
        )
        return "allowed", 0, 0, DEFAULT_USER_LIMIT
    ...
    try:
        ...
        generation_count = 0
    try:
        # Preserve manually changed limits, such as an existing value of 10.
        max_limit = int(data.get("max_limit", DEFAULT_USER_LIMIT))
    except (TypeError, ValueError):
        max_limit = DEFAULT_USER_LIMIT

    return "allowed", access_count, generation_count, max_limit
...
def _increment_usage(email, field_name, timestamp_field=None):
    email = _normalize_email(email)

    doc = _get_usage_document(email)
    if doc is None:
        # Recovery path if a document was deleted between login checks.
        get_user_stats(email)
        doc = _get_usage_document(email)
    if doc is None:
        raise RuntimeError(f"No user_usage document found for {email}")

    data = doc.to_dict()
    try:
        current_value = int(data.get(field_name, 0))
    except (TypeError, ValueError):
        current_value = 0

    update_data = {field_name: current_value + 1}
    if timestamp_field:
        update_data[timestamp_field] = firestore.SERVER_TIMESTAMP
    db.collection(FIRESTORE_COLLECTION).document(doc.id).update(update_data)
    _record_firebase_status(
        f"Firestore updated user_usage/{doc.id} in project "
        f"{firebase_project_id or 'UNKNOWN_PROJECT'}: {field_name} incremented for {email}."
    )


def increment_access(email):
    try:
        _increment_usage(email, "access_count", "last_accessed_at")
    except Exception as exc:
        st.error(f"Firebase access counter update failed: {type(exc).__name__}: {exc}")
        st.exception(exc)
        raise
...
def increment_generation(email):
    try:
        _increment_usage(email, "generation_count", "last_generated_at")
    except Exception as exc:
        st.error(f"Firebase generation counter update failed: {type(exc).__name__}: {exc}")
        st.exception(exc)
        raise
...
def show_login_page():
    ...
    if firebase_init_error:
        st.warning("Firebase usage tracking is unavailable in this deployment. Login cannot be completed until FIREBASE_SERVICE_ACCOUNT is configured in Streamlit Secrets.")
        ...
        if st.session_state.get('show_otp_input', False):
            ...
            if st.button("Verify OTP", use_container_width=True, key="verify_otp_btn"):
                if otp_input == st.session_state.otp_code:
                    verified_email = _normalize_email(st.session_state.get("otp_email"))
                    try:
                        allowed, lim, acc, gen = check_access_allowed(verified_email)
                        if allowed:
                            # Count only a successful, Firebase-backed login.
                            increment_access(verified_email)
                        ...
                        st.error(
                            "Login could not be completed because Firebase usage "
                            f"tracking failed: {type(exc).__name__}: {exc}"
                        )
                        st.exception(exc)
...
if not st.session_state.authenticated:
    ...
    firebase_status = st.session_state.pop("firebase_status_message", None)
    if firebase_status:
        st.success(firebase_status)
    st.caption(
        f"Firebase project: {firebase_project_id or 'UNKNOWN_PROJECT'} | "
        f"Firestore collection: {FIRESTORE_COLLECTION}"
    )
